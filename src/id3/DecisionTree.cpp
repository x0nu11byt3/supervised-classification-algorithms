...
void DecisionTree::printNode(const Node* node, const std::string& prefix, bool isLast) const {
    if (!node) return;

    // Print the prefix and the connection symbol
    std::cout << prefix;
    if (isLast) {
        std::cout << "└── ";
    } else {
    ...
    if (node->isLeaf) {
        std::cout << "Class: " << node->predictedClass << std::endl;
    } else {
        std::cout << "Attribute " << node->featureIndex << " <= " << node->threshold << "?" << std::endl;

        // Determine the new prefix for the children
        std::string newPrefix = prefix + (isLast ? "    " : "│   ");

        // Print left child (if any)
        if (node->left) {
            // The left-hand son will be the youngest if there is no right-hand son
            bool leftIsLast = (node->right == nullptr);
            printNode(node->left.get(), newPrefix, leftIsLast);
        }

        // Print right child (if any)
        if (node->right) {
            // The right-hand son is always the youngest
            printNode(node->right.get(), newPrefix, true);
        }
    }
}
